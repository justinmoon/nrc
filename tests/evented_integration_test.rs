...
impl EventedTestClient {
    pub async fn new(name: &str) -> Result<Self> {
        ...
        let temp_dir = std::env::temp_dir().join(format!("evented_test_{}_{}", name, rand::random::<u32>()));
...
async fn test_evented_two_account_dm_flow() -> Result<()> {
    ...
    assert!(!alice_messages.is_empty(), "Alice should have received Bob's message");
    ...
    let alice_reply = bob_messages.iter().find(|m| m.content.contains("Nice to hear from you"));
    assert!(alice_reply.is_some(), "Bob should have received Alice's reply");
