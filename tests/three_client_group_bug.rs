...
async fn test_three_client_group_message_visibility() -> Result<()> {
    ...
    alice.execute_command(&format!("/create #test {bob_npub}")).await?;
    ...
    // Check for any error message
    {
        ...
        if let Some(ref flash) = alice_nrc.flash_message {
            assert!(flash.contains("Created #test"), "Should confirm group creation: {}", flash);
        } else {
    ...
    assert_eq!(charlie.group_count().await, 1, "Charlie should have joined the group");
    ...
    // CRITICAL CHECK: Alice should still be able to see Charlie's message
    {
        ...
        for msg in &messages {
            println!("  - {}: {}", 
                msg.sender.to_bech32().unwrap_or_else(|_| "unknown".to_string()), 
                msg.content
            );
        }
