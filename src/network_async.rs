        ...
        for group_id in groups {
            ...
            if client.subscribe(filter.clone(), None).await.is_ok() {
                if let Ok(events) = client.fetch_events(filter, Duration::from_secs(2)).await {
                    ...
                    log::debug!("Fetched {} events for group {}", events_vec.len(), h_tag_value);
                    all_events.extend(events_vec);
                }
            }
        }
        
        log::debug!("Background: fetched {} total message events in {:?}", all_events.len(), start.elapsed());
...
pub fn spawn_fetch_welcomes(
    ...
    tokio::spawn(async move {
        ...
        if let Ok(events) = client.fetch_events(filter, Duration::from_secs(2)).await {
            let events_vec: Vec<Event> = events.into_iter().collect();
            log::debug!("Background: fetched {} GiftWrap events in {:?}", events_vec.len(), start.elapsed());
